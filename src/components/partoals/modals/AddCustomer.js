...
const AddCustomer = ({setModalShow, ...props}) => {
    ...
    const handlecustomerCreate = () =>{
        setIsLoading(true)
    axios.post(`${Constants.BASE_URL}/customer`, input)
      .then(res => {
        setIsLoading(false)
        Swal.fire({
            position: 'top-end',
    ...
    return (
        ...
        >
            ...
            <Modal.Footer>
            <button onClick={handlecustomerCreate} className={"btn theme-button"}
                          dangerouslySetInnerHTML={{ __html: isLoading ? '<span className="spinner-grow spinner-grow-sm" role="status" aria-hidden="true"></span> Loading...' : 'Add Customer' }}
                        />
            </Modal.Footer>
